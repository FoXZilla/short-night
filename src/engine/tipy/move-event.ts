...
export default class MoveEvent{
    ...
    static avoid({mover, fixed, direction}:{mover:Component, fixed:Component, direction:1|-1}){
        if(direction > 0){
            mover.drawInfo.box.y += fixed.drawInfo.box.y + fixed.drawInfo.box.height - mover.drawInfo.box.y + 1;
        }else{
            mover.drawInfo.box.y -= mover.drawInfo.box.y + mover.drawInfo.box.height - fixed.drawInfo.box.y + 1;
        }
        mover.apply();
    };
    static isConflict(eb1:Component, eb2:Component){
        return eb1!==eb2 && isOverlap(eb1.drawInfo.box,eb2.drawInfo.box)
    }
    ...
    public async walkOn(){
        ...
        repair: while(true){
            switch(await this.tryFixOne()){
                case Result.NoConflict:
                    return Result.NoConflict;
                case Result.Alleviated:
                    alleviated = true;
                    continue repair;
                case Result.Failed:
                    return alleviated
     ...
     * */
    private async tryFixOne() :Promise<Result>{
        this.countConflict();
        this.countSpace();

        console.log(this.conflicts);

        if(this.conflicts.length === 0) return Result.NoConflict;
        this.conflicts = this.conflicts.filter(
            conflict => this.isPossible(conflict)
        );
        if(this.conflicts.length === 0) return Result.Failed;
    ...
    private isPossible(conflict:Conflict){
        ...
        return (needed.bottom === 0 || needed.top === 0)
            && (space.top >= needed.bottom && space.top >= needed.bottom)
        ;
    };
    private async fixConflict(conflict:Conflict){
        ...
        // and fix side-effect
        for(
            let i = this.allEventBody.indexOf(conflict.self) + direction;
            i>=0 && i<this.allEventBody.length;
            i += direction
        ){
            const last = this.allEventBody[i - direction];
            const now = this.allEventBody[i];
        ...
        for(let eb of this.allEventBody){
            const conflict = {
                ...
                with: this.tipy.components[SN.EventBody].filter(
                    target=>MoveEvent.isConflict(eb, target)
                ) as EventBody[],
            };
    ...
    private countNeeded(conflict:Conflict) :{top:number, bottom:number}{
        ...
        if(above.length){
            result.top = Math.max(
                ...above.map(
                    cb => cb.drawInfo.box.y + cb.drawInfo.box.height - origin.drawInfo.box.y
                )
            );
        }
        if(below.length){
            result.bottom = Math.max(
                ...below.map(
                    target => origin.drawInfo.box.y + origin.drawInfo.box.height - target.drawInfo.box.y
                )
            );
        }
    ...
    private countSpace(){
        ...
        const allEventBody = Array.from(this.allEventBody);

        // Itself's can move space
        allEventBody.forEach(eventBody=>this.spaceMap.set(eventBody, {
            bottom: eventBody.drawInfo.target.y - eventBody.drawInfo.box.y - spacePadding,
            top: eventBody.drawInfo.box.y + eventBody.drawInfo.box.height - eventBody.drawInfo.target.y - spacePadding,
            // bottom: eventBody.drawInfo.box.y + eventBody.drawInfo.box.height - eventBody.drawInfo.target.y - spacePadding,
        }));

        // And it is limited by neighbors
        ...
        this.spaceMap.get(last)!.top = Math.min(
            ...
            (
                this.tipy.components[SN.AxisBody][0].drawInfo.box.y
                + this.tipy.components[SN.AxisBody][0].drawInfo.box.height
                - last.drawInfo.target.y
                - (last.drawInfo.box.y + last.drawInfo.box.height - last.drawInfo.target.y)
            ),
        );
